...
//!
//! Implements the HTTP Signatures draft spec
//! (<https://tools.ietf.org/html/draft-cavage-http-signatures>).
//!
//! These are the *pure* signing primitives — no HTTP, no platform traits — so they
//! are shared verbatim by `dais-core` (server) and the `dais` client. The one
//! HTTP-coupled helper, `fetch_actor_public_key`, stays in `dais-core` where the
//! `HttpProvider` abstraction lives.
