...
- [x] **Node.js/npm** - External runtime for the Cloudflare Wrangler CLI only
- [x] **wrangler CLI** (3.0+) - Cloudflare's deployment tool, installed outside this repository
...
Install Wrangler outside the Dais checkout, then verify it is available on
`PATH`. Wrangler is Cloudflare's deployment CLI and is distributed through the
Node package ecosystem; it is not Dais application code.
...
`build/worker/shim.mjs` is generated by Rust `worker-build`. It is Cloudflare's
module wrapper for the compiled Rust/WASM worker, not checked-in application
JavaScript and not an npm dependency.
