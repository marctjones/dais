...
Public results include action metadata when available: `actions`, `watch_type`,
`watch_target`, `reply_target`, and `follow_target`. Clients can use these fields
to offer Watch, Reply, Follow, Open, and ActivityPub Like/Boost actions without
guessing protocol-specific target formats.
